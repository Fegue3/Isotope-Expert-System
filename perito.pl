...
continua :-
    retract(executa(_,1):-_),  % já carregámos uma BC
    esperaOrdens(23).
...
soluciona :-
    ...
    objectivo(X),
    nl, nl, write('Resposta encontrada: '), write(X), nl, nl.
...
questiona(Atributo,Valor) :-
    write(Atributo:Valor), write('? (sim/nao) '),
    read(R),
...
processa(R,Atributo,Valor) :-
    asserta(conhece(R,Atributo,Valor)), !, fail.
